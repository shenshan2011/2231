...
(async function authGuard() {
  ...
  renderOverview();
  renderSysInfo();
  window._sysInfoInterval = setInterval(renderSysInfo, 10000);
})();
...
window.modLogout = async function() {
  // Clear sysInfo interval to prevent memory leak after logout
...
window.showTopicAttempts = function(userId, topicId) {
  ...
  window._topicAttemptsTopicId = topicId;
...
window.showTopicAttemptByIndex = async function(i) {
  const attMeta = (window._topicAttempts||[])[i];
  if (!attMeta) return;
  const userId  = window._topicAttemptsUserId;
  const username = _allProfiles.find(p=>p.id===userId)?.username||'?';

  // Fetch data lengkap (dengan answers) karena _allProgress sudah di-strip
  document.getElementById('answerModalContent').innerHTML =
    '<div style="padding:40px;text-align:center;color:var(--mod-muted)">⏳ Memuat jawaban…</div>';

  const fullRow  = await getFullProgress(userId);
  const topicId  = window._topicAttemptsTopicId;
  const fullAttempts = (fullRow.quiz_attempts||{})[topicId] || [];
  const fullAtt  = fullAttempts.find(a => a.attemptId === attMeta.attemptId) || attMeta;

  _showAttemptDetail({...fullAtt, username});
};
...
function renderSysInfo() {
  ...
  if (!el) { // Elemen tidak ada (setelah navigasi) — stop interval
    if (window._sysInfoInterval) { clearInterval(window._sysInfoInterval); window._sysInfoInterval = null; }
    return;
  }

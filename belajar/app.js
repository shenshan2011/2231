...
const AUTH = (() => {
  ...
  return {
    ...
    async register(username, password) {
      ...
      // FIX: Pakai @example.com — reserved domain RFC 2606, diterima semua validator email
    ...
    async login(username, password) {
      const u     = username.trim().toLowerCase();
      // FIX: Harus konsisten dengan register — pakai @example.com
      const email = `${u}@example.com`;
...
function updateMenuPage() {
  ...
  const elAv   = $('menuAvatar');
  const elUn   = $('menuUsername');
  const elXp   = $('menuXP');
  const elMod  = $('menuModeratorCard');
  const elTI   = $('menuThemeIcon');
  const elTL   = $('menuThemeLabel');
  const elMI   = $('menuMuteIcon');
  const elML   = $('menuMuteLabel');

  if (elAv)  elAv.textContent  = uname.charAt(0).toUpperCase();
  if (elUn)  elUn.textContent  = uname;
  if (elXp)  elXp.textContent  = '⚡ ' + xp.toLocaleString('id-ID') + ' XP';
  if (elMod) elMod.style.display = role === 'moderator' ? '' : 'none';

  const isDark  = document.documentElement.classList.contains('dark');
  const isMuted = localStorage.getItem('bc-mute') === '1';
  if (elTI) elTI.textContent = isDark  ? '☀️' : '🌙';
  if (elTL) elTL.textContent = isDark  ? 'Tema Terang' : 'Tema Gelap';
  if (elMI) elMI.textContent = isMuted ? '🔇' : '🔊';
  if (elML) elML.textContent = isMuted ? 'Suara Off'   : 'Suara On';
}
...
function toggleTheme() {
  ...
  // Update icon di menu page jika sedang terbuka
  if ($('page-menu')?.classList.contains('active')) updateMenuPage();
}
...
function renderTKAQuestion(index) {
  ...
  const badgeEl = $('tkaTopicBadge'); if (badgeEl) {
    const lvlRaw = (q.level || '').toLowerCase();
    const lvlMap = { mudah: 'Mudah', sedang: 'Sedang', susah: 'Sulit', sulit: 'Sulit' };
    const lvl = lvlMap[lvlRaw] || q.level || '';
    badgeEl.textContent = `${q.topic} • ${lvl}`;
  }

...
function startTKA(subject) {
  ...
  // Filter soal yang belum ada fotonya
  const availablePool = TKA_DATA[subject].filter(q => !q.needsImage);
  const skipped = TKA_DATA[subject].length - availablePool.length;
  if (skipped > 0) console.info(`[TKA] ${skipped} soal dilewati karena foto belum tersedia.`);

  tkaSubject = subject; tkaAnswers = {}; tkaMarked = new Set(); tkaCurrentIndex = 0; tkaStartTime = Date.now();
  const cfg = TKA_CONFIG[subject]; const pool = [...availablePool];
